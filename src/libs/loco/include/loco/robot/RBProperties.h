 ...
 */
class RRBCollisionHeightMap : public RRBCollsionShape {
public:
    RRBCollisionHeightMap(const unsigned char* data, double width, double depth, int nWidth, int nDepth, int nChannel, double heightScale, double offset)
        : data(data), width(width), depth(depth), nWidth(nWidth), nDepth(nDepth), nChannel(nChannel), heightScale(heightScale), offset(offset) {}
    ~RRBCollisionHeightMap() override = default;

    float getHeight(int nx, int nz) const {
        const unsigned char* y = data + (nx + nWidth * nz) * nChannel;
        return y[0];
    }

public:
    const unsigned char* data = nullptr;
    const int nWidth, nDepth;  // number of samples along width and depth axes
    const int nChannel;        // in case data has multiple channel
    const double width, depth, heightScale, offset;
};
